...
function displayDate(s){return /^\d{4}-\d{2}-\d{2}$/.test(s||'')?`${s.slice(8,10)}/${s.slice(5,7)}/${s.slice(0,4)}`:(s||'')}
function isoDate(s){if(/^\d{4}-\d{2}-\d{2}$/.test(s||''))return s;const d=String(s||'').replace(/\D/g,'');if(d.length!==8)return '';const iso=`${d.slice(4,8)}-${d.slice(2,4)}-${d.slice(0,2)}`,x=new Date(iso+'T00:00:00');return !Number.isNaN(x.getTime())&&x.toISOString().slice(0,10)===iso?iso:''}
function maskDateInput(el){const d=el.value.replace(/\D/g,'').slice(0,8);el.value=[d.slice(0,2),d.slice(2,4),d.slice(4,8)].filter(Boolean).join('/')}
function syncProjectEndDate(){const s=$('#pstart'),e=$('#pend');if(!s||!e)return;maskDateInput(e);if(s.value){const endIso=isoDate(e.value);if(!endIso||endIso<s.value)e.value=displayDate(s.value)}}
function openProject(pid=''){const p=data.projects.find(x=>x.id===pid)||{};$('#projectModalTitle').textContent=pid?'Sửa dự án':'Tạo dự án';$('#pid').value=p.id||'';$('#pname').value=p.name||'';$('#pstatus').value=p.status||'planning';$('#pdesc').value=p.description||'';$('#ppriority').value=p.priority||'medium';$('#pbudget').value=p.budget||0;$('#pstart').value=p.start||new Date().toISOString().slice(0,10);$('#pend').value=displayDate(p.end||'');syncProjectEndDate();$('#pleader').value=p.leaderId||data.members[0]?.id||'';$('#pprogress').value=p.progress||0;$('#pProgressText').textContent=(p.progress||0)+'%';openModal('projectModal')}
function saveProject(e){e.preventDefault();syncProjectEndDate();const old=data.projects.find(p=>p.id===$('#pid').value),p={id:$('#pid').value||id('p'),code:old?.code||code('DA',data.projects),name:$('#pname').value.trim(),status:$('#pstatus').value,description:$('#pdesc').value.trim(),priority:$('#ppriority').value,budget:+$('#pbudget').value||0,start:$('#pstart').value,end:isoDate($('#pend').value),leaderId:$('#pleader').value,progress:+$('#pprogress').value||0};data.projects=old?data.projects.map(x=>x.id===old.id?p:x):[p,...data.projects];closeModals();persist(old?`Đã cập nhật dự án: ${p.name}`:`Đã tạo dự án: ${p.name}`)}
