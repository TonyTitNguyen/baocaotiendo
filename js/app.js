...
function members(){ $('#members').innerHTML=data.members.map(m=>`<article class="member"><div class="avatar">${init(m.name)}</div><h3>${clean(m.name)}</h3><p>${clean(m.role||'Thành viên')}</p></article>`).join('') }
