...
function setProjectEndDate(iso=''){const d=$('#pendDay'),m=$('#pendMonth'),y=$('#pendYear');if(!d||!m||!y)return;if(validIsoDate(iso)){d.value=iso.slice(8,10);m.value=iso.slice(5,7);y.value=iso.slice(0,4)}else{d.value='';m.value='';y.value=''}}
function getProjectEndDate(){const d=String($('#pendDay')?.value||'').padStart(2,'0'),m=String($('#pendMonth')?.value||'').padStart(2,'0'),y=String($('#pendYear')?.value||'');const iso=`${y}-${m}-${d}`;return validIsoDate(iso)?iso:''}
