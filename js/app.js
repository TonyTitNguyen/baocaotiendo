...
function members(){ if($('#memberTotal'))$('#memberTotal').textContent=data.members.length;$('#members').innerHTML=data.members.map(m=>{const assigned=data.tasks.filter(t=>t.assigneeId===m.id).length,leading=data.projects.filter(p=>p.leaderId===m.id).length;return `<article class="member"><div class="member-top"><div class="avatar">${init(m.name)}</div><button class="tiny d member-delete" title="Xóa thành viên" onclick="delMember('${m.id}')">${trashIcon}</button></div><h3>${clean(m.name)}</h3><p>${clean(m.role||'Thành viên')}</p><div class="member-meta"><span>${assigned} việc</span><span>${leading} dự án</span></div></article>`}).join('')||'<div class="empty">Chưa có thành viên</div>'}
...
function validIsoDate(iso){const x=new Date(iso+'T00:00:00');return /^\d{4}-\d{2}-\d{2}$/.test(iso||'')&&!Number.isNaN(x.getTime())&&x.toISOString().slice(0,10)===iso}
function setProjectEndDate(iso=''){const d=$('#pendDay'),m=$('#pendMonth'),y=$('#pendYear'),legacy=$('#pend');if(legacy)legacy.value=validIsoDate(iso)?iso:'';if(!d||!m||!y)return;if(validIsoDate(iso)){d.value=iso.slice(8,10);m.value=iso.slice(5,7);y.value=iso.slice(0,4)}else{d.value='';m.value='';y.value=''}}
function getProjectEndDate(){const legacy=$('#pend');if(!$('#pendDay')||!$('#pendMonth')||!$('#pendYear'))return legacy?.value||'';const d=String($('#pendDay')?.value||'').padStart(2,'0'),m=String($('#pendMonth')?.value||'').padStart(2,'0'),y=String($('#pendYear')?.value||''),iso=`${y}-${m}-${d}`,out=validIsoDate(iso)?iso:'';if(legacy)legacy.value=out;return out}
function handleDatePartInput(el,next){el.value=el.value.replace(/\D/g,'').slice(0,el.maxLength);if(el.value.length>=el.maxLength&&next)next.focus()}
function syncProjectEndDate(){const s=$('#pstart');if(!s?.value)return;const hasAny=!!($('#pendDay')?.value||$('#pendMonth')?.value||$('#pendYear')?.value),endIso=getProjectEndDate();if(!hasAny||(endIso&&endIso<s.value))setProjectEndDate(s.value)}
function openProject(pid=''){const p=data.projects.find(x=>x.id===pid)||{};$('#projectModalTitle').textContent=pid?'Sửa dự án':'Tạo dự án';$('#pid').value=p.id||'';$('#pname').value=p.name||'';$('#pstatus').value=p.status||'planning';$('#pdesc').value=p.description||'';$('#ppriority').value=p.priority||'medium';$('#pbudget').value=p.budget||0;$('#pstart').value=p.start||new Date().toISOString().slice(0,10);setProjectEndDate(p.end||'');syncProjectEndDate();$('#pleader').value=p.leaderId||data.members[0]?.id||'';$('#pprogress').value=p.progress||0;$('#pProgressText').textContent=(p.progress||0)+'%';openModal('projectModal')}
function saveProject(e){e.preventDefault();syncProjectEndDate();const old=data.projects.find(p=>p.id===$('#pid').value),p={id:$('#pid').value||id('p'),code:old?.code||code('DA',data.projects),name:$('#pname').value.trim(),status:$('#pstatus').value,description:$('#pdesc').value.trim(),priority:$('#ppriority').value,budget:+$('#pbudget').value||0,start:$('#pstart').value,end:getProjectEndDate(),leaderId:$('#pleader').value,progress:+$('#pprogress').value||0};data.projects=old?data.projects.map(x=>x.id===old.id?p:x):[p,...data.projects];closeModals();persist(old?`Đã cập nhật dự án: ${p.name}`:`Đã tạo dự án: ${p.name}`)}
